...
Tein poiston komennolla `sudo apt-get purge apache2`. Poisto onnistui ongelmitta, mutta koska kansiot /var/www/html/ ja /etc/apache2/sites-available/ eivät olleet tyhjiä, niitä ei terminaalin mukaan poistettu. Seuraavaksi uudelleen asennus komennolla `sudo apt-get install apache2`.
...
Seuraavaksi tallensin tehdyt muutokset erikako.conf -tiedostoon, ja toivoin että kaikki olisi viimein kunnossa. Kävin toisessa terminaalissa varmistamassa, mikä pythonin site-packages -kansion polku on (lähinnä että mikä versionumero siinä on, aiemmin olen senkin laittanut väärin).

![image](https://github.com/user-attachments/assets/a1b4bfbe-a568-471c-8ba5-99dbaa1c24bb)

Ajoin uudestaan WSGI-moduulin asennuskomennon, vaikka se minulta jo löytyikin. 

![image](https://github.com/user-attachments/assets/c8395d2b-894a-4814-ba73-cfdfa4f8b42e)

Komennolla `/sbin/apache2ctl configtest` sain esiin tutun herjan AH00558, mutta syntaksin pitäisi olla OK. Apachen uudelleenpotkaisu käyntiin ja sormet ristissä curlilla kokeilu. Sentään ei enää 500 Internal Server Erroria, mutta edelleen vanha /var/www/html/index.html -tiedosto nousee esiin.

![image](https://github.com/user-attachments/assets/5234dc58-8e8f-4178-9fb6-f31f4634b1a1)

Oletin, että joku ei vieläkään toimi, koska ohjeessa kerrotaan, että tässä kohtaa pitäisi tulla Djangon oma sivu, eikä mikään oma html-tiedosto. Tarkistin näkymän vielä nettiselaimen puolelta, ja siellä näkyy tietysti sama sivu mikä curlissa, eli itse tekemäni html. Apache pyörii oikein, mutta eikö sitten mod_wsgi ole toiminnassa?

![image](https://github.com/user-attachments/assets/be2aae00-51fa-42a3-8571-a642aef313e8)

Yritin miettiä, millä lähtisin ongelmaani ratkomaan, sillä en oikein tiedä mikä on vialla. Stack Overflown keskustelusta (lähde: Stack Overflow, https://stackoverflow.com/questions/23890486/how-to-check-if-mod-wsgi-is-installed-on-a-shared-apache-server) löysin komennon, jolla voin tarkistaa onko mod-wsgi toiminnassa, ja se näytti olevan ihan kunnossa, kunhan tajusin lisätä "-py3" -lopun.

![image](https://github.com/user-attachments/assets/2774537b-5844-4a83-a7f1-799bc59ba030)

Päätin kysyä asiaa chatGPT:ltä, koska olen täysin jumissa. En usko että tehtävää kannattaa mennä eteenpäin, ennenkuin voin olla varma että mod_wsgi toimii kunnolla. Kysyin chatGPT:ltä, miten voisin lähteä selvittämään sitä, miksi curl localhost palauttaa tiedoston /var/www/html/ -kansiosta, eikä Djangon omaa sivua. ChatGPT neuvoi seuraavat askeleet; 1. Varmista, että mod_wsgi on asennettu ja otettu käyttöön, 2. Konfiguroi Apache osoittamaan Django-sovellukseesi, 3. Salli oikeudet projektin hakemistoihin, 4. Tarkista Apache-lokit ja 5. Varmista, että Django on oikein konfiguroitu tuotantoon. 

Lähdin etenemään ohjeiden mukaan, ja varmistin vielä uudelleen mod_wsgi-moduulin asennuksen ja että Apache käyttää sitä. Käynnistin samalla Apachen uudestaan.

![image](https://github.com/user-attachments/assets/5602a011-fe96-4be9-9eba-da530466fffe)

Tässä välissä testaus komennolla `curl -s localhost|grep title`, mutta edelleen omatekemä oletussivu näkyvillä.

Seuraavaksi lähdin tarkastelemaan erikako.conf -tiedostoa. ChatGPT:n ohjeet eivät ihan sopineet Teron ohjeista kopioituihin konfiguraatioihin, mutta luin tarkaan tiedoston uudestaan läpi, enkä huomannut virheitä. Oikeuksia en uskaltanut lähteä muokkaamaan, vaan oletin että niiden pitäisi olla kunnossa, eli sen osan skippasin tekoälyn luomasta ohjeesta.

Seuraavaksi lokien kimppuun. Ajoin vielä kertaalleen terminaalissa komennon `curl -s localhost|grep title`, jonka jälkeen avasin lokit komennolla `sudo tail -f /var/log/apache2/error.log`. 

![image](https://github.com/user-attachments/assets/dc04343f-f319-41c1-851e-31953a4a08aa)

Ajoin toisessa terminaalissa curlin vielä uudestaan, eikä siitä ainakaan mitään virherivejä muodostunut. Googletin notice-numeroita (AH00094, AH00489 ja AH00489), mutta en saanut yhdestäkään mitään korjattavaa ongelmaa. Käynnistin apachen uudestaan, ja sain samoilla notice-numeroilla 3 uutta riviä error.logiin, eli selkeästi varsinaista virhettä ei tapahdu, vaan lokiin merkkaantuu uudelleenkäynnistys.

Viimeisenä kohtana ChatGPT:n ohjeessa oli varmistaa Djangon konfiguroinnit tuotantoon, eli että onko Djangon asetustiedostossa settings.py sallittu localhost hostina. Kävin kurkkaamassa, mitä tiedosto pitää sisällään. Kohdassa ALLOWED_HOSTS ei ollut mitään, joten kokeilin lisätä siihen localhostin. Uudelleenkäynnistin Apachen, mutta curl näyttää edelleen väärää sivua. Otin juuri tehdyn muutoksen settins.py -tiedostosta pois.

> Klo 21:21, väliaikatieto keskellä jumia

En pääse etenemään, en tiedä miten ongelmaa lähtisin vielä selvittämään. 

Tehtävänannossa seuraavana tehdään samaiseen settings.py -tiedostoon muutoksia, joten kokeilen vielä sitä. Muutin tiedostossa DEBUG = False ja ALLOWED_HOSTS = ["localhost", "hello.erikakoskinen.com"]. Käynnistin Apachen uudelleen, mutta ei vieläkään pelitä. Ohjeen mukaan näkyviin pitäisi tulla Not Found, mutta minulla näkyi edelleenkin oma oletussivuni. 

![image](https://github.com/user-attachments/assets/2eeab6a3-4f73-45d3-9b09-32d8a29e86df)

Muutin asetukset takaisin. Kävin katsomassa, onko .conf -tiedostoissa vielä jotain häikkää, ja että onhan varmasti 000-default.conf -tiedosto disabloitu. Löysin samalla kansion /etc/apache2/sites-enabled/, jota kävin uteliaisuudesta katsomassa. Siellä oli vain koskinene.conf, joka oli static-tehtävässä. Heräsi epäilys, että eikö erikako.conf -tiedosto ollutkaan otettu käyttöön. Ajoin komennon sen käyttöönottoon, käynnistin Apachen uudestaan ja tarkistin kansion sisällön. Kappas vaan kun nyt lähtikin toimimaan... Tarkistin myös selaimessa, ja sielläkin näkyi Djangon sivu ihan oikein.

![image](https://github.com/user-attachments/assets/f8e65fac-222c-4497-854e-51b213505d0a)

> Klo 21:52, viimeiset silaukset

Takaisin settings.py -tiedostoon, ja muutin tehtävänannossa sanotut kaksi kohtaa. Tallensin tiedoston ja käynnistin Apachen uudestaan.

![image](https://github.com/user-attachments/assets/71a4a57b-9f22-4c9d-a05e-0fdb40bb88da)

Nyt curlilla testaus näyttää taas "oikein", että sivua ei löydy, koska mitään ei ole vielä määritelty.

![image](https://github.com/user-attachments/assets/0aa61c32-0b08-421e-b4aa-5532b64c1f94)

Nettiselaimessa kuitenkin sivu http://localhost/admin toimii, ja näyttää vähän heikon näköistä kirjautumissivua.

![image](https://github.com/user-attachments/assets/ef9aaf0e-56a9-41f0-884f-39ee3398bce0)

Takaisin settings.py -tiedostoon tekemään tarvittavat lisäykset, jotta saadaan kirjautumissivusta hieman nätimpi.

![image](https://github.com/user-attachments/assets/0dcf581d-f0e3-4541-b695-756fef31987c)

Ohjeen mukainen komento `./manage.py collectstatic` tuotti kasan virheilmoituksia.

![image](https://github.com/user-attachments/assets/d4e13d04-2324-4c5a-a371-5c37d27bbae3)

Kävin vaihtamassa STATIC_ROOTin paikan BASE_DIR -rivin alle setting.py -tiedostossa, jotta BASE_DIR määritellään ensin.

![image](https://github.com/user-attachments/assets/c7acbca5-3937-46c2-9a5b-69a6f472e466)

Nyt komento `./manage.py collectstatic` onnistui.

![image](https://github.com/user-attachments/assets/707bf144-de69-48b2-af89-ec6841ca792f)

Selaimessa kirjautumisikkuna oli muuttunut toivotulla tavalla.

![image](https://github.com/user-attachments/assets/1df496ea-3a5d-414e-a010-b3ca7c9cede5)

En päässyt kirjautumaan ensimmäisessä tehtävässä tekemilläni tunnuksilla, sillä ne olivat eri Django-projektia varten. Loin siis uuden admin-tunnuksen tähän projektiin. Loin ensin salasanan komennolla `pwgen -s 20 1`, ja yritin luoda adminia komennolla `./manage.py createsuperuser`, mutta sain vastaukseksi pitkän virhelistan, käyttämättömistä migraatioista. Ajoin komennon `python manage.py migrate`, jonka jälkeen käyttäjän luominen onnistui.

![image](https://github.com/user-attachments/assets/5e63e751-b704-4fa3-ab6c-c372320a1929)

Vielä jäljellä kirjautumisen testaus, ja kaikki VIIMEIN kunnossa.

![image](https://github.com/user-attachments/assets/dec0f100-12ae-4a71-92d8-6cd410445db2)

> Klo 22:15, viikon tehtävät purkissa vaikka kovasti koittivat hangoitella vastaan
